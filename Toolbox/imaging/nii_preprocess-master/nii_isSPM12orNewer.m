...
r = str2double(r);
if (r < 7771)
    [~,spm_folder,~] = fileparts(fileparts(which('spm')));
    if strcmp(spm_folder,'brainageR_SPM')
        error('brainageR SPM detected. Please restart pipeline pathing or run allengit_genpath')
    else
        error('%s: please update SPM12', mfilename); 
    end
end
if (r > 7592) && (r < 7965)
