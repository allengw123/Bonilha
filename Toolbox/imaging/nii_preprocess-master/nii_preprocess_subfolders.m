function nii_preprocess_subfolders(pth,outpath)
...
isUpdateVers = false; %set LIME version to latest
isCopymat = true; %copy output files to M.vers and LIME.vers folders
isMakeModalityTable = false;
...
if isCopymat
    ...
    mkdir(outpth);
    parfor i = 1: numel(f) %change 1 to larger number to restart after failure
       cpth = char(deblank(f(i))); %local child path
       %fprintf('===\t%s participant %d/%d : %s\t===\n', mfilename, i, numel(f), cpth);
