...
if [ ${#dtir} -eq 0 ]; then  #only given a single DTI
	...
	echo $indx > $dti_txt2
	echo $eddyExeName --imain=$dti --mask=$dti_b --acqp=$dti_txt --index=$dti_txt2 --bvecs=$dti_bvec --bvals=$dti_bval --repol --out=$dti_u --data_is_shelled
	time $eddyExeName --imain=$dti --mask=$dti_b --acqp=$dti_txt --index=$dti_txt2 --bvecs=$dti_bvec --bvals=$dti_bval --repol --out=$dti_u --data_is_shelled
