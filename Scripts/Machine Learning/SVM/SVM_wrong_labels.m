...
save_path='C:\Users\bonilha\Documents\Project_Eleni\SVM_results_all\SVM_results_wrong_labels';
mkdir(save_path);
...
function [SVMModel,accuracytraining,accuracytesting]=runsvm(ROI_nii_log,control_path,patient_path,ROI_name,patient_name,save_path)
    ...
    for svm_count=1:100
        ...
        % Train SVM
        SVMModel{svm_count,1}= fitcsvm(array2table(ROI_traindata.all), ROI_traindata_wrong_labels,'KernelFunction','linear','KFold',5);
