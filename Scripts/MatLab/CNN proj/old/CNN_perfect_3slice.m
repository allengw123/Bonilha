...
for con=1:numel(control_nii)
    ...
    control_img_2(:,:,1,con)=temp.img(:,:,50);
    control_img_3(:,:,1,con)=temp.img(:,:,80);
end
...
for pat=1:numel(patient_nii)
    ...
    patient_img_2(:,:,1,pat)=temp.img(:,:,50);
    patient_img_3(:,:,1,pat)=temp.img(:,:,80);
end
...
for iter=1:10
    display(['Processing Iteration ',num2str(iter)])
    %% Permute
