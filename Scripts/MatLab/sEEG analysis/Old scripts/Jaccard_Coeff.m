...
for sbj = 1:size(subjID,1)
    ...
    for fv = 1:numel(funcvars)
        ...
        % Load data
        tempfv=load(fullfile(wk_structfolder,funcvars{fv}));
    ...
    for freq=1:numel(freq_bands)
        ...
        cohmats=dir(fullfile(wk_functionalfolder,['*',freq_bands{freq},'*']));
        cohmats(contains({cohmats.name},'baseline')) = [];
            ...
            for fv=1:numel(funcvars)
                for clip=1:numel(cohmats)
                    for phas=1:numel(phases)
                        ...
                        jac_coeff.(freq_bands{freq}).(thres_name{thres}).(wk_sbjID).(funcvars{fv}){clip}(phas)=jaccard(tempfunc,tempstruct);
                        ...
                        % Save label matrix
                        electrode_comp.(freq_bands{freq}).(thres_name{thres}).(wk_sbjID).(funcvars{fv}){clip}{phas}=double(electrode_idx);
...
respdat=cell2mat(tempdat(responsive_idx));
nonrespdat=cell2mat(tempdat(nonresponsive_idx));

figure;
plot(mean(respdat,1))
hold on
plot(mean(nonrespdat,1))
legend({'Resp', 'NonResp'})
