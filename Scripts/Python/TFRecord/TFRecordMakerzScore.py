...
def TFcreate(file):
    v_filename, disease, label = file[0]
    z_v_filename, disease, label = file[1]
    ...
    # The volume, in nifti format	
    z_nii = nib.load(z_v_filename)
    # The volume, in numpy format
    z_np = z_nii.get_fdata().astype(np.float32)

    # Stack t1 map and z map
    vz_nii = np.stack((v_np,z_np),3)
    ...
    data_point = tf.train.Example(features=tf.train.Features(feature={
        'image': _float_feature(vz_nii.ravel()),
...
zscore_dir = r'F:\PatientData\LargeSet_4_7\zScore'
matter = 'GM'

out_dir  = r'F:\PatientData\LargeSet_4_7\TFRecords_LTLE_RTLE_T1_zScore'
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
    
    
preproc_files = list(listfiles(preproc_dir,'volume'))
zscore_files = list(listfiles(zscore_dir,'zScore'))

REF_DIM = (113, 137, 113)
num_cores = multiprocessing.cpu_count()

[TFcreate(i) for i in zip(preproc_files,zscore_files)]
