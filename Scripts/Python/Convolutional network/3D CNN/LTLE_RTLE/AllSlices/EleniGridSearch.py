...
def train_test_model(dimensions, hparams, run_dir):
    ...
    reduce_lr =  tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5,
                      patience=10, min_delta=0.0001, min_lr=0.000001)
    
    # TensorBoard Callback
    tb_Callback = TensorBoard(log_dir=run_dir)
    ...
    history = model.fit(trainingDataset,
              ...
              shuffle=True,
              callbacks =  [reduce_lr, LRTensorBoard(log_dir=os.path.join(run_dir)), tb_Callback, ES_Callback]
              )
    metric_output = []
    ...
    acc,F1 = confusionMat(test_weights,test_predictions,label)
    ...
    metric_output.append(acc[2])
    metric_output.append(F1[0])
    metric_output.append(F1[1])
    metric_output.append(F1[2])
...
def run(run_dir, hparams):
  with tf.summary.create_file_writer(run_dir).as_default():
    ...
    for i in range(len(METRIC)):
        tf.summary.scalar(METRIC[i], model_metric[i], step=2)
...
HPARAM_DIR = r'F:\CNN output\3D_CNN\RTLE_LTLE_Healthy\AllSlices\models\hparam\BatchSize\hparam_tuning'
...
HP_BATCHSIZE = hp.HParam('BatchSize', hp.Discrete(np.arange(1,50).tolist()))
...
session_num = 0
for b in HP_BATCHSIZE.domain.values:
