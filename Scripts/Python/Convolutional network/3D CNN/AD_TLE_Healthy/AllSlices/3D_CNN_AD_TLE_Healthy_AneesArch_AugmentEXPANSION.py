...
def PreprocImg(input):
    ...
    Augvol = []
    for sbj in range(imgs.shape[3]):
        wk_vol = imgs[:,:,:,sbj]
        print('Creating Augmented images for subject ' + input[sbj])
        for p in perm:
            temp_volume = ndimage.rotate(wk_vol,angle=p[0],axes=(0,1),reshape=False)
            temp_volume = ndimage.rotate(temp_volume,angle=p[1],axes=(0,2),reshape=False)
            temp_volume = ndimage.rotate(temp_volume,angle=p[2],axes=(1,2),reshape=False)
